...
| Agent documentation, context, and Git operating system | active | The policy gate validates the actual event commit range as well as project/PR context; representative ALM policy and container runs pass, while the full candidate cohort, immutable-review-head confirmation, and external rollout remain | [PR #22](https://github.com/MartinHaghani/ALM/pull/22), [issue #1](https://github.com/MartinHaghani/ALM/issues/1), and the [active migration ExecPlan](exec-plans/active/agent-operating-system-migration.md) |
