...
These settings start the passive SLAM manager, a SLAM-only local odometry helper, and the passive alignment helper used by the combined `/next/` map. Mapping starts stopped by default so the operator can choose the first map origin from the WebUI. When mapping is started, the manager resets the SLAM-only odometry origin and starts `slam_toolbox`; the alignment helper clears old samples and learns a visualization-only `map -> slam_map` transform. It prefers saved mowing-boundary samples from `/area_recorder/boundary_samples`, then falls back to synchronized RTK-fixed `map -> base_link` and `slam_map -> slam_base_link` motion samples if no usable boundary path exists. Boundary samples are now plain `base_link` pose pairs (GPS-fused vs. SLAM) so the alignment math is independent of the area-recorder's rake selection. Boundary samples default to a larger retention window than generic motion samples so long yard recordings are not truncated, and the status topic sends a decimated calibration trace for the WebUI. Live GPS and SLAM robot status poses are time-synchronized when their TF stamps are close enough, reducing motion-only marker separation caused by GPS/fusion latency. The default passive tree while mapping is `map -> slam_map -> slam_odom -> slam_base_link -> slam_lidar`; the raw C1 scan from `/hw/lidar` is republished as `/slam_toolbox/scan` in `slam_lidar`. The mower's existing `map -> base_link -> lidar` localization remains separate and authoritative for normal mower behavior, so passive SLAM does not feed costmaps, planning, or control.

Area recording sweeps an evenly-spaced rake of body-frame points along the entire right or left side of the costmap footprint (default 9 points each side, configurable via `mower_logic/area_recording_rake_points`; the front-most, geometric center, and rear-most points are always included). Each 10 Hz tick, the recorder projects all rake points to the map frame and accepts only the one whose world position projects furthest outward along the perpendicular-right (or perpendicular-left) of the mower's recent travel direction; the result is then min-distance-gated and pruned for inward notches on finish. This lets a zero-turn mower capture reflex outdoor corners by driving past and pivoting, and 90-degree internal corners by driving in and reversing, without requiring the front-right corner of the chassis to thread the boundary perfectly. Mowing outlines use the right-side rake, obstacle outlines use the left-side rake, and navigation outlines remain anchored at `base_link`. Polygon recording still skips new points unless the raw GPS pose is RTK fixed and within `/xbot_positioning/max_gps_accuracy`. Existing maps must be cleared and rerecorded after enabling the rake-based recording so the saved polygons match the new outermost-point semantics.
