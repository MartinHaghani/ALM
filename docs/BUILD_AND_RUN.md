...
- `open_mower.launch` includes passive C1 SLAM only when `OM_USE_PASSIVE_SLAM=True`; that launch path now starts the passive SLAM manager, SLAM-only odometry helper, and GPS/LIDAR alignment helper for `/next/` visualization. The alignment helper pairs `base_link` poses between GPS-fused and SLAM frames (boundary samples no longer carry a footprint offset), so changes to the area-recorder rake do not destabilize the rigid-transform fit. For best alignment, clear old mower/SLAM maps after deploying rake-based boundary recording and rerecord mowing outlines by driving the mower's right side along the boundary CCW (or CW; winding is detected). The area recorder skips new polygon points while the raw GPS pose is not RTK fixed or exceeds the configured GPS accuracy limit, and selects the outermost rake point along the right side per tick so reflex corners and 90-degree internal corners can be captured by pivoting and reversing instead of threading a single chassis corner.
