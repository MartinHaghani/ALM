...
- `docker/Dockerfile.PiDev` derives from `ros:noetic-ros-base-focal` and installs this repo's apt-level ROS dependencies for a local-checkout workflow on the Pi.
- `start_open_mower_local.sh` launches through the repo's `docker/openmower_entrypoint.pi.sh` inside the bind-mounted checkout.
- `docker/openmower_entrypoint.pi.sh` installs small missing runtime libraries when needed, then delegates to `docker/openmower_entrypoint.legacy.sh` so legacy `mower_config.sh` values become the `MOWER`, `ESC_TYPE`, and related runtime environment expected by `open_mower.launch`.
