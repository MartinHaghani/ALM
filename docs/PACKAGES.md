...
- `src/open_mower`: orchestration package. It provides launch files, parameter YAML, hardware-specific presets, RViz configs, the raw TCP RTCM bridge script used by the main comms launch flow, and a `terrain_replay.launch` harness for steep-slope bag playback.
...
- `src/mower_msgs`: shared mower-specific ROS messages and services such as `Status`, `Power`, `HighLevelStatus`, `TerrainState`, and `HighLevelControlSrv`.
...
- `src/lib/ftc_local_planner`: follow-the-carrot local planner plugin with dynamic reconfigure, debug topics, recovery behavior, and the terrain-aware speed/gain/lookahead adaptation path driven by `mower_logic/terrain_state`. This fork's mower defaults also enable obstacle checking against the map edge during mowing and use a tighter `max_follow_distance` than upstream so the controller bails out sooner instead of wandering far off the planned path before declaring a crash.
...
- `src/lib/xbot_monitoring`: monitoring package providing `xbot_monitoring`, `heatmap_generator`, and an example sensor node. In this fork it also republishes terrain telemetry into retained `robot_state/json`, and its teleop bridge suppresses redundant neutral `Twist` frames so an idle connected client does not keep `twist_mux` pinned away from autonomous navigation.
- `src/lib/xbot_msgs`: shared xBot message and service package, including the extended `RobotState` message used to surface terrain mode, slope, slip, and terrain-memory risk in the UI path.
