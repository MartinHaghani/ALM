...
- `src/open_mower/scripts/localization_fusion.py`: first-party read-only shadow fusion publisher for `/next/`. It fuses the operational `/xbot_positioning/xb_pose` base pose with the globally aligned passive LIDAR pose, while using raw GPS only to require RTK fixed before GPS can contribute. If GPS drops to RTK float, GPS is ignored and the shadow pose follows LIDAR-only when LIDAR is globally available.
